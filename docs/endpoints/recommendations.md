...
Сгенерировать рекомендации по питанию за последние 7 локальных дней.

Backend:

1. определяет текущую локальную дату пользователя по `X-Timezone`;
2. берет период `сегодня - 6 дней` → `сегодня`;
...
```json
{
  "text": "За последнюю неделю видно, что обеды самые калорийные...",
  "meals_analyzed": 18,
  "period": {
    "from": "2026-06-13",
    "to": "2026-06-19"
